class colorfulConfigureDialog extends UIScriptedMenu
{
	...
	override Widget Init()
	{
		...
		layoutRoot = GetGame().GetWorkspace().CreateWidgets("CUT_Mainmenu/gui/layouts/new_ui/dialogs/cui.Configure.dialog.layout" );
	...
	// Applies all pending option changes, then closes this dialog and the options menu.
	void ApplyAndExit()
	{
		OptionsMenu optionsMenu = OptionsMenu.Cast( GetGame().GetUIManager().FindMenu( MENU_OPTIONS ) );

		GetGame().GetUIManager().Back(); // close this dialog first

		if ( optionsMenu )
		{
			optionsMenu.Apply();

			// If applying popped a modal (e.g. restart required), let the user handle it first
			if ( !GetGame().GetUIManager().IsDialogVisible() && !GetGame().GetUIManager().IsModalVisible() )
			{
				optionsMenu.Back();
			}
		}
